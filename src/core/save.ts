...
export function collectlLabelalikeUpdates(
    ...
    removedPlaceholderF : (key : string) => (null | LAType) ) : { [landCode : string] : LAType | null } | null {

  const oldData : { [landCode : string] : LAType } = originalEntity[ labelalikeType ] || EMPTY_OBJECT;
  const newData : { [landCode : string] : LAType } = entity[ labelalikeType ] || EMPTY_OBJECT;
  ...
  if ( changes.length > 0 ) {
    const result : { [langCode : string] : LAType | null } = {};
