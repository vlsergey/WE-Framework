...
import type { BaseEditorPropsTypes } from './BaseEditorPropsTypes';
...
export default class BaseWikibaseItemDataValueEditor<PropsType : BaseEditorPropsTypes, StateType>
  ...
  handleCreate( entityId ) {
    ...
    onDataValueChange( {
      ...
      type: BaseWikibaseItemDataValueEditor.DATAVALUE_TYPE,
    } );
  }
  ...
  render() {
    ...
    const className = styles[ 'wef_datavalue_' + BaseWikibaseItemDataValueEditor.DATATYPE ];

    if ( readOnly ) {
      return <td
        className={className + ' ' + styles[ 'wef_datavalue_' + BaseWikibaseItemDataValueEditor.DATATYPE + '_readonly' ]}
        colSpan={12}>
