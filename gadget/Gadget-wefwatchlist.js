( function() {
	...
	function getWikidataApiPrefix() {
		return '//www.wikidata.org/w/api.php' + '?origin=' + encodeURIComponent( location.protocol + mw.config.get( 'wgServer' ) );
	}
	...
	function insertChange( headers, jEntry ) {
		...
		if ( typeof headers[date] !== 'undefined' ) {
			var jH4 = headers[date];
			var time = pad( updated.getHours() ) + ':' + pad( updated.getMinutes() );

			var toInsertBefore = null;
			var div = jH4.find( '~ div' ).first();
			$.each( div.children(), function( c, child ) {
				if ( toInsertBefore !== null ) {
					return;
				}

				var jChild = $( child );
				var timeOfElement = $( jChild.find( 'td.mw-enhanced-rc' )[0] ).text().substring( 6 ).trim();
	...
	function generateWatchlistLine( jEntry, time ) {
		...
		$( document.createElement( 'td' ) ).addClass( 'mw-enhanced-rc' ).html(
				"<abbr class=\'wikidata\' title=\'" + i18n.linePartTooltip + "\'>" + i18n.linePartLetter + "</abbr>&#160;&#160;&#160;&#160;&#160;" + time + "&#160;" ).appendTo(
				firstLine );
		...
		mainLine
				.append( $( document.createElement( 'a' ) ).addClass( 'mw-userlink' ).addClass( 'userlink' ).text( author ).attr( 'href', '//www.wikidata.org/wiki/User:' + author ) );
		mainLine.append( $( document.createElement( 'span' ) ).addClass( 'mw-changeslist-separator' ).text( '. .' ) );

		firstLine.append( mainLine );

		var secondLine = $( document.createElement( 'tr' ) ).appendTo( table ).css( 'display', 'none' );
